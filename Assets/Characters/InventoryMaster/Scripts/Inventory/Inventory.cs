...
public class Inventory : MonoBehaviour
{
    ...
    public static event ItemDelegate HotBarItemChanged;
    ...
    public static void checkIfAllInventoryClosed()
    {
        ...
        for (int i = 0; i < canvas.transform.childCount; i++)
        {
            ...
            if (child.activeSelf && 
                (child.tag == "EquipmentSystem" || child.tag == "Panel" || child.tag == "MainInventory" || child.tag == "CraftSystem")
                && !child.name.Contains("Hotbar")) return;
    ...
    public void NotifyHotBarItemChanged(Item item)
    {
        if (HotBarItemChanged != null)
            HotBarItemChanged(item);
    }
    ...
    public GameObject addItemToInventory(int id, int value)
    {
        ...
        var itemType = itemDatabase.getItemByID(id).itemType;
        var equipmentSystem = GetComponent<EquipmentSystem>();
