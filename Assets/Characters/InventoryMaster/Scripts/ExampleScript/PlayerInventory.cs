...
public class PlayerInventory : MonoBehaviour
{
    ...
    */
    ...
    public void OnEnable()
    {
        ...
        Inventory.HotBarItemChanged += ChangeAttackType;
    }
    ...
    void ChangeAttackType(Item item)
    {
        //nemoze mijenjat oruzje dok napada
        if (PlayerManager.PlayerScript.IsAttacking) return;
        
        if (item.itemType == ItemType.Weapon)
        {
            foreach (var itemAttr in item.itemAttributes)
            {
                if (itemAttr.attributeName == "Sword")
                {
                    PlayerManager.PlayerScript.Type = Player.PlayerType.Sword;
                    PlayerManager.PlayerScript.SetActiveSword(itemAttr.attributeValue);
                }
                else if (itemAttr.attributeName == "Bow") PlayerManager.PlayerScript.Type = Player.PlayerType.Bow;

            }

        }
        else if (item.itemType == ItemType.Magic) PlayerManager.PlayerScript.Type = Player.PlayerType.Magic;

        foreach(var itemAttr in item.itemAttributes){
            if (itemAttr.attributeName == "AttackType") PlayerManager.PlayerScript.AttackIndex = itemAttr.attributeValue;
        }
        /*
        if (item.itemType == ItemType.Magic)
        {
            PlayerManager.PlayerScript.DoAttack();
        }
        */
    }
